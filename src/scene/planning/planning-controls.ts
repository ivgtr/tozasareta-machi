...
import { derivePlanningStatus } from './model'
...
export class PlanningControls extends Phaser.GameObjects.Container {
  ...
  update(state: GameState, plan: PlanState, busy: boolean, selectedAssigned: boolean): void {
    ...
    const summary =
      this.deviceClass === 'wide'
        ? `${status.remaining === 0 ? '配置完了' : `待機 ${status.remaining}人`}  /  予算 −${status.plannedBudget}${status.plannedStockpile > 0 ? `  備蓄 −${status.plannedStockpile}` : ''}`
        : `${status.remaining === 0 ? '配置完了' : `待機 ${status.remaining}人`}  予算−${status.plannedBudget}`
    ...
    text.setPosition(SPACING.md, this.deviceClass === 'wide' ? 12 : 6)
    d.add(text)
  }
  ...
  private layoutWide(): void {
    ...
    this.rationButton.setPosition(62, 52)
    this.procureButton.setPosition(174, 52)
    this.autoButton.setPosition(62, 100)
    this.removeButton.setPosition(62, 100)
    this.commitButton.setPosition(148 + (w - 148) / 2, 100)
  }
