...
describe('settlement', () => {
  it('食料はユニット数×単価だけ消費される', () => {
    const s = base()
    const { state } = settle(s, { ration: false, worked: [] })
    expect(state.resources.food).toBe(s.resources.food - s.units.length * BALANCE.unit.foodPerUnit)
  })

  it('配給を絞ると消費が半分になる', () => {
    const s = base()
    const { state } = settle(s, { ration: true, worked: [] })
    expect(state.resources.food).toBe(
      s.resources.food - Math.round((s.units.length * BALANCE.unit.foodPerUnit) / 2),
    )
  ...
  it('頑丈なユニットは決して負傷しない', () => {
    for (let i = 0; i < 40; i++) {
      ...
      const r = settle(s, { ration: false, worked: workedFarmer }).state // farmer は sturdy
  ...
  it('医療不足では働くユニットがいずれ負傷する', () => {
    ...
    for (let i = 0; i < 40 && !injured; i++) {
      ...
      const r = settle(s, { ration: false, worked: workedFarmer }).state
  ...
  it('働くと成長し、閾値で適性が上がる', () => {
    ...
    for (let i = 0; i < BALANCE.unit.growthThreshold; i++) {
      s = settle(s, { ration: false, worked: workedFarmer }).state
    }
  ...
  it('士気が低いといずれ離脱が起きる', () => {
    ...
    for (let i = 0; i < 40 && !deserted; i++) {
      ...
      const r = settle(s, { ration: false, worked: [] }).state
