...
import {
  deriveEndingNarrative,
  deriveEndingPresentation,
} from '../src/scene/story/ending-model'
...
describe('deriveEndingPresentation', () => {
  ...
  it('同じ完全復旧でも犠牲・協力・避難者受入で結末文を変える', () => {
    ...
    expect(new Set([plain?.outcome, cooperative?.outcome, refugees?.outcome, casualty?.outcome]).size).toBe(
      4,
    )
