...
- **アート導入中**: イベント・エンディング・タスク/状態/資源アイコン・背景(`scene/night.png`)の PNG を導入済み。ユニット肖像画は未（計画: `docs/14-汎用ユニット肖像画計画.md`）
- **テスト68件 緑 / CI 緑**
...
| `ending/` | 導入済み（4枚） |
| `icon/` | タスク+状態+資源アイコン導入済み。資源4枚（food/power/medical/morale）はスロット配置済みだが、現行の StatusWall はテキスト表示 |
...
2. **資源アイコンのUI接続**（food/power/medical/morale の PNG は配置済み。StatusWall への表示は未）
