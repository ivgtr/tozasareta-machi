...
- **予定アート導入完了**: イベント・エンディング・タスク/状態/資源アイコン・背景(`scene/night.png`)・肖像（ユニーク30+汎用8）・スカイラインの PNG を導入済み
- **選択型イベント 実装完了（docs/16 P1+P2）**: 決定フェーズ（`phase:'choice'`/`resolveChoice`）+ 交易/危機/探索の選択イベント。探索は備蓄前払いで送出・数ターン不在（労力不可）・毎日抽選で帰還・適性で大成功(4日分)/成功(1日分)/危険/失敗。士気→生産倍率・任務係数引き締めで経済をイベント主軸へ
- **テスト88件 緑 / CI 緑**
