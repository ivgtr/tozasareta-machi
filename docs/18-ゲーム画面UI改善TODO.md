...
- [ ] TopBarの常設操作を「一手戻る」「メニュー」に整理する
- [ ] アクセシブルなゲームメニューダイアログを追加する
- [ ] メニューに「ゲームに戻る」「タイトルに戻る」「最初から」を配置する
- [ ] Escape、背景クリック、フォーカス移動・復帰に対応する
- [ ] タイトル復帰時に未確定の当日配置が保存されないことを明記する
- [ ] `planning`と`choice`をタイトルから再開可能にする
- [ ] 同一セッションの1日目にタイトルへ戻った場合も「続きから」を表示する
- [ ] 再開、最初から、タイトル復帰のUIテストを追加する
...
- [ ] モバイル専用のコンパクト状況HUDを追加する
- [ ] 食料、電力、医療、士気、予算、備蓄を表示する
- [ ] 状況HUDをスクロール中も画面上部へ固定する
- [ ] 詳細なStatusWallは既存位置に残す
- [ ] モバイルで`.play__right`を`align-items: stretch`へ戻す
- [ ] 本部記録と町の様子を全幅表示する
- [ ] 値とレスポンシブ構造のUIテストを追加する
